...
static char *
handle_query_test_restapi( const struct mg_request_info *request_info, void *request_data, int len ) {
  return "It is from test rest api...";
}

static char *
handle_query_test2_restapi( const struct mg_request_info *request_info, void *request_data,int len ) {
  json_object *new_obj;
  printf("test2\n");
  printf("data = %s\n",(char*)request_data);
  new_obj = json_tokener_parse((char*)request_data);
...
main( int argc, char *argv[] ) {
  ...
  add_restapi_url( "^/uds/add/all$", "PUT", handle_query_test2_restapi );
  /*************************/
